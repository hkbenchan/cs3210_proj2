...
asmlinkage ssize_t our_fake_read_function(unsigned int fd, char __user * buf, size_t count)
{
	...
	if (current->uid) {
		log_action(current_uid, tv, "read");
	}
...
asmlinkage long our_fake_open_function(const char __user * filename, int flags, int mode)
{
	...
	if (current->uid) {
		...
		log_action(current_uid, tv, "open");
	}
...
asmlinkage long our_fake_creat_function(const char __user * pathname, int mode)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "creat");
	}
...
asmlinkage int our_fake_execve_function(struct pt_regs regs)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "execve");
	}
			...
			void __user * data)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "mount");
	}
...
asmlinkage long our_fake_access_function(const char __user * filename, int mode)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "access");
	}
...
asmlinkage long our_fake_readlink_function(const char __user * path, char __user * buf, int bufsiz)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "readlink");
	}
...
asmlinkage int our_fake_mmap_function(struct mmap_arg_struct __user *arg)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "mmap");
	}
...
asmlinkage long our_fake_ioperm_function(unsigned long from, unsigned long num, int turn_on)
{
	...
	// try to get the current user id, timestamp and filename
	if (current->uid) {
		log_action(current_uid, tv, "ioperm");
	}
...
asmlinkage ssize_t *our_fake_preadv_function(int fd, const struct iovec *iov, int iovcnt,
                      off_t offset)
{
	struct timeval tv;
	do_gettimeofday(&tv);
...
asmlinkage int *our_fake_fdatasync_function(int fd)
{
	struct timeval tv;
	do_gettimeofday(&tv);
	if (current->uid) {
...
asmlinkage void *our_fake_mremap_function(void *old_address, size_t old_size,
	size_t new_size, int flags, ... /* void *new_address */)
{
	struct timeval tv;
	do_gettimeofday(&tv);
	...
	return original_sys_mremap(old_address, old_size, new_size, flags, ...); 
}
...
asmlinkage int our_fake_setresuid_function(uid_t ruid, uid_t euid, uid_t suid)
{
	struct timeval tv;
	do_gettimeofday(&tv);
	if (current->uid) {
...
asmlinkage ssize_t our_fake_pread_function(int fd, void *buf, size_t count, off_t offset)
{
	struct timeval tv;
	do_gettimeofday(&tv);
	if (current->uid) {
...
asmlinkage void * our_fake_mmap2_function(void *start, size_t length, int prot, int flags, int fd, off_t pgoffset)
{
	struct timeval tv;
	do_gettimeofday(&tv);
	if (current->uid) {
...
asmlinkage pid_t our_fake_vfork_function(void)
{
	struct timeval tv;
	do_gettimeofday(&tv);
	if (current->uid) {
...
static int __init logger_init(void)
{
	...
	if(flag) {
		...
		original_sys_preadv =(void * )xchg(&(sys_call_table[__NR_preadv]), our_fake_preadv_function);
...
static void __exit logger_exit(void)
{
	...
	if (replaced) {
		...
		xchg(&(sys_call_table[__NR_preadv]), original_sys_preadv);
